...
import GLProgram from "../utils/gl-program";
import getGLContext from "../utils/get-gl-context";
...
export default function FluidAnimation({ opts = {}, config }) {
  const { color: userColors } = opts;
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      if (canvas.width !== width || canvas.height !== height) {
        (canvasRef.current || { width: 0, height: 0 }).width = width;
        (canvasRef.current || { width: 0, height: 0 }).height = height;
      }
    }
  ...
  const initFramebuffers = (gl, ext) => {
    ...
    textureWidthRef.current =
      gl.drawingBufferWidth >> defaultConfig.textureDownsample;
    textureHeightRef.current =
      gl.drawingBufferHeight >> defaultConfig.textureDownsample;
